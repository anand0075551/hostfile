        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                <div class="box-header">
                    <h3 class="box-title">Assign To Other Warehouse</h3>
                </div><!-- /.box-header -->
                <!-- form start -->
                <?php echo form_open_multipart('', ['role' => 'form', 'class' => 'form-horizontal']); ?>
			 	...
			 	<div class="form-group <?php if (form_error('product_id')) echo 'has-error'; ?>">
								...
								<div class="col-md-9">  								
									...
									<select name="product_id" id="product_prep" style="width:100% auto;" onchange="get_unique_pack(this.value)" class="form-control">
										...
										<?php 
								$get_product1 = $this->db->group_by('product')->get_where('inventory_movement');
								foreach($get_product1->result() as $p){
									$product_id = $p->id;
									$product = $p->product;
									
									
								$get_declared_name = $this->db->get_where('smb_product', ['id'=>$product]);
									foreach($get_declared_name->result() as $t);
									$declared_name = $t->title;
									
									echo "<option value='".$product."'>".$declared_name."</option>";
								}
							?>
									</select>
							...
							<div class="form-group <?php if (form_error('assigned_role')) echo 'has-error'; ?>">
								...
								<div class="col-md-9">  								
									...
									<select name="assigned_role"  class="form-control" style="width:100% auto;"onchange="get_user1(this.value)">
										...
										<option value="89"> Other Warehouse Keeper </option>
									</select>	                           
				 ...
				 <div class="form-group <?php if (form_error('assigned_to_name')) echo 'has-error'; ?>">
                    ...
                    <div class="box-footer">
						<button type="submit" name="submit" id="submit" value="othr_ware" class="btn btn-primary">
...
<script>
...

	{
		...
		$.ajax({
			type:"POST",
			url:"<?php echo base_url('Product_preparation/get_unique_pack11') ?>",
			data:mydata,
	...
	function get_unk_prep(id)
{
	...
	$.ajax({
	 ...
	 $.ajax({
                type: "POST",
				url: "<?php echo base_url('product_preparation/get_button_disabled_asg_Other_ware') ?>",
                data: mydata,
                success: function (response) {
