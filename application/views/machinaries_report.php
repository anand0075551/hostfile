				...
				<div class="row" style="padding:10px;">
					...
					<div class="col-sm-3">
						...
						<select class="form-control" name="name" id="name" style=" width:100% auto; ">
							<option value="">Choose Machine Name.</option>
							...
							<?php 
								...
								{
									$get_users = $this->db->order_by('id' ,'ASC')->group_by('name')->get_where('machinaries');
								}
						...
						<select class="form-control" name="vehicle_id" id="vehicle_id" style=" width:100% auto; ">
							...
							<?php 
								...
								if($currentUser=='admin')
								{
									$get_users = $this->db->order_by('id','DESC')->group_by('vehicle_id')->get_where('machinaries');
								}
								...
								foreach($get_users->result() as $p){
									...
									echo "<option value=".$p->vehicle_id.">".$p->vehicle_id."</option>";
								}
							?>
						</select>
					</div>
			...
			<div class="box-header" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); ">
				<div class="row" style="padding:10px;">
					<div class="col-sm-12 text-right">
						...
						&nbsp;
						<button type="button" class="btn btn-primary btn-sm btn-flat" onclick="CSV.begin('#example2').download('machinaries report.csv').go()" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); "><i class="fa fa-file-excel-o"></i> Download CSV</button>
					</div>
				</div>
			</div><!-- /.box-header -->
			<!-- Search table-->
...
<script>
	...
	function createPDF(){
		getCanvas().then(function(canvas){
			...
			doc.save('machinaries report.pdf');
