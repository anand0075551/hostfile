...
<section class="content">
    <div class="row">
        ...
        <div class="col-md-12">
                ...
                <?php echo form_open_multipart('', ['role' => 'form', 'class' => 'form-horizontal']); ?>
                 ...
                 <p><label> Category</label></p>
                       ...
                       <p><label>Item Name</label></p>
							...
							//$get_status = $this->db->get_where('inventory_movement', ['product_unique_code'=>$inventory_stocks->product_unique_code,'created_by'=>$user_id]);
			
				$get_status = $this->db->get_where('inventory_movement', ['common_serial'=>$inventory_stocks->common_serial,'created_by'=>$user_id]);
			
			                   if($get_status->num_rows() > 0)
			               {
							   foreach ($get_status->result() as $weight){
				               $final = $weight->left_pieces;
							   }
			                    }
								 else{
									 
									
				               $final = $inventory_stocks->pieces;
			...
			<div class="col-md-4"> 
                 <p><label>Price for Pieces</label></p> 	 								
									<input type="text" name="price_of_pieces"  class="form-control" onkeyup="get_total()" id="price_pieces" value ="0" readonly >                             
									<?php echo form_error('quantity') ?>
...
?>		
							<input type="hidden" name="assigned_by"  id="assigned_by" value="<?php echo $product_warehouse_assigned->created_by;?>">
							
							<input type="hidden" name="assigned_to" value="<?php echo $product_warehouse_assigned->assigned_to_name;?>" id="assigned_to">
	...
	function get_mfr_date(id)
{
	...
	function get_total(){
	    ...
	    var price_pieces       = $("#price_per_unit").val();
	...
	var type1 = parseFloat(type,10);
	 ...
	 var pieces = parseFloat(quantity,10);
	  ...
	  var piece_pp        = parseFloat(price_pieces,10);
	 
	  
	 var total_price_add  = parseFloat(price_pieces,10) * parseFloat(quantity1,10);
	...
	$("#price_pieces").val(total_price_add); 
