										...
										<tbody>
											 <?php
												...
												<tr>
													...
													<?php 
														echo $row1['name']."<br>"; 
														if(singleDbTableRow($row1['id'],'smb_stock')->hsn_code != ""){
															echo "HSN Code : ".singleDbTableRow($row1['id'],'smb_stock')->hsn_code;
														}
														$name = $row1['name'];
													?>
													<?php
...
?>										
										<tbody>
											<?php
												$total_tax = 0;
												foreach ($product_details as $pro) {
													$vendor_id = $pro['vendor'];	
													if($vendor_id==$user_id){
														$total_tax += ($pro['qty']*$pro['tax']);
													}
												}
											?>
											<?php
										...
										<tbody>
											...
											<tr style="background:lavender; border-top:2px solid black;  border-bottom:2px solid black;">
												<th>Total</th>
												<th class="text-right"><?php echo number_format(($sub_total+$total_tax+$total_shipping),2); ?></th>
											</tr>
										</tbody>
									</table>
								</div>
