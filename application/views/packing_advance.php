			...
			<!--	 <div class="form-group <?php if (form_error('product_preparation')) echo 'has-error'; ?>">
				 ...
				 <input type="hidden" name="product_name" id="prod_name">
						...
						<div class="form-group <?php if (form_error('assigned_role')) echo 'has-error'; ?>">
								<label for="firstName" class="col-md-3">Packing Category
									<span class="text-red">*</span>
								</label>
								<div class="col-md-9">  								

									<select name="vv"  class="form-control" style="width:100% auto;"onchange="get_packs(this.value)">
										<option value=""> Choose option </option>
										<?php
										if ($packing_categ->num_rows() > 0) {
											foreach ($packing_categ->result() as $c) {

											   echo '<option value="'.$c->packing_category_id.'"> '.$c->packing_category.'</option>';
											}
										}
										?>
									</select>	                                
							...
							<div class="form-group <?php if (form_error('product_preparation')) echo 'has-error'; ?>">
								<label for="firstName" class="col-md-3">Pack Type
									<span class="text-red">*</span>
								</label>
								<div class="col-md-9">  								
							...
							onchange="get_total_output(this.value)"
							<!--===================-->							
							<select class="form-control" name="package_name_small" id="pack_type" onchange="get_pack_weight(this.value)"style=" width:100% auto; ">
			...
			<!--		<?php/* if (!empty ($pack_now) && $pack_now->num_rows() > 0)
					...
					*/
					?> -->
						...
						<tr>
							<td>1</td>
							<td><input type="text" name="pre_def_small" id="pre_def_small" onkeyup="get_total()" class="form-control" readonly></td>
						...
						<tr style="display:none">
							...
							<td><input type="text" name="pieces_medium"  id="pieces_medium" onkeyup="get_total()" class="form-control" value="0" placeholder="Enter No. Of Pieces"></td>
							<td><input type="text" name="weight_medium" id="weight_medium" onkeyup="get_total()" class="form-control" value="0"readonly></td>
						...
						<tr style="display:none">
							...
							<td><input type="text" name="pieces_large" id="pieces_large"  onkeyup="get_total()" class="form-control" value="0" placeholder="Enter No. Of Pieces"></td>
							<td><input type="text" name="weight_large" id="weight_large" onkeyup="get_total()" class="form-control" value="0"readonly></td>
						...
						<tr style="display:none">
							...
							<td><input type="text" name="pieces_family" id="pieces_family" onkeyup="get_total()" class="form-control" value="0" placeholder="Enter No. Of Pieces"></td>
							<td><input type="text" name="weight_family" id="weight_family" onkeyup="get_total()" class="form-control" value="0"readonly></td>
						...
						<tr style="display:none">
							...
							<td><input type="text" name="pieces_combo" id="pieces_combo" onkeyup="get_total()" class="form-control" value="0" placeholder="Enter No. Of Pieces"></td>
							<td><input type="text" name="weight_combo" id="weight_combo" onkeyup="get_total()" class="form-control" value="0"readonly></td>
			 ...
			 <div class="box-footer">
                <button type="submit" name="submit" id="submit" value="add_advance_packing" class="btn btn-primary">
...
<script>
	function get_packs(id)

	{
	  
     //alert(id);
		var mydata = {"packing_category_id":id};
		$.ajax({
			type:"POST",
			url:"<?php echo base_url('Product_preparation/get_packs') ?>",
			data:mydata,
			success:function(response){
				//alert(response);
				$("#pack_type").html(response);
			}
		});	
}
...
<script>
...

	{
     ...
     //alert(id);
		...
		$.ajax({
			type:"POST",
			url:"<?php echo base_url('Product_preparation/get_pack_weight') ?>",
			data:mydata,
			success:function(response){
				//alert(response);
				$("#pre_def_small").val(response);
			}
		});	
}
