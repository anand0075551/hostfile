					...
					<table id="deposit" class="table table-bordered table-striped table-hover">
							...
							<div id="flip"> <h4>Click here to See the Conversion Ratio</h4></div>
								...
								<table class="table table-bordered">
								  ...
								  <tr> <th>  Below Convertion Ratio Points are calculated based on your Available Balance Wallet/Cash </th></tr>
							<!-- 	   <tr> <td> Maximum Eligible for ' <?php echo ($loyality);   ?> ' Loyality Points	 </td></tr>
								   <tr> <td>Maximum Eligible for ' <?php echo ($discount);   ?>	' Discount Points </td></tr>
								 -->    <tr><th> Loyality...( Wallet 1 =   <?php echo ($ratio_loyality ) ; ?>) Points 		</th> <td> Maximum Eligible for ' <?php echo number_format($loyality, 2);   ?> ' Loyality Points	 </td></tr>
								   <tr><th> Discount..( Wallet 1 =   <?php echo ($ratio_discount ) ; ?>) Points		</th> <td>Maximum Eligible for ' <?php echo number_format($discount, 2);   ?>	' Discount Points </td></tr>
						</table>
							</div>
							<hr />

                  <div class="form-group <?php if(form_error('conv_type')) echo 'has-error'; ?>">
                        <label for="firstName" class="col-md-3">Type of Cash Points
                            <span class="text-red">*</span>
                        </label>
						<div class="col-md-9">
                        <!--    <select name="conv_type" class="form-control">
                                <option value=""> Select The Points Type to Convert </option>
								<option value="wallet" 		<?php echo set_select('conv_type', 'wallet')    ?>>Wallet/Cash</option>
								<option value="loyality" 	<?php echo set_select('conv_type', 'loyality') 	?>>Loyality</option>                                
                                <option value="discount" 	<?php echo set_select('conv_type', 'discount')  ?>>Discount</option>
                               
                            </select>	-->	 
							<input type="radio" name="conv_type"  <?php if (isset($conv_type) && $conv_type=="wallet") ;?> value="wallet" checked id="wallet" /> Wallet/Cash        
						...
						<div class="form-group <?php if(form_error('trans_type')) echo 'has-error'; ?>">
							...
							<input type="radio" name="trans_type"  <?php if (isset($trans_type) && $trans_type=="wallet") ;?> value="wallet" id="wallet" /> Wallet/Cash 		 
