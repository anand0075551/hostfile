		...
		<div class="box">
			<div class="box-header" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); ">
				<div class="row" style="padding:10px;">
					...
					?>
					<div class="col-sm-3">
						<p><label>Store Id</label></p>
						<select class="form-control" name="declared_by" id="declared_by" style=" width:100% auto; ">
							<option value="">Choose option</option>
							<?php 
							
								$get_declared_name = $this->db->get_where('inventory_store_id');
									foreach($get_declared_name->result() as $p)
									{
									$declared_name = $p->store_name.' '.$p->area;
									
									echo "<option value='".$p->id."'>".$p->id.'-'.$declared_name."</option>";
									}
							?>
						</select>
					</div>
			...
			<div class="box-header" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); ">
				<div class="row" style="padding:10px;">
					<div class="col-sm-12 text-right">
						...
						&nbsp;
						<button type="button" class="btn btn-primary btn-sm btn-flat" onclick="CSV.begin('#example').download('Assigned Products.csv').go()" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); "><i class="fa fa-file-excel-o"></i> Download CSV</button>
					</div>
				</div>
			</div><!-- /.box-header -->
...
<script type="text/javascript">
	$(function() {
		$("#example").dataTable({
			"processing": true,
			"serverSide": true,
			"ajax": "<?php echo base_url('Product_preparation/product_assigned_ListJson'); ?>"
		});
	});
...
<script>
	...
	function createPDF(){
		getCanvas().then(function(canvas){
			...
			doc.save('Assigned Products.pdf');
