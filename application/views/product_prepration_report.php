		...
		<div class="box">
			<div class="box-header" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); ">
				<div class="row" style="padding:10px;">
					...
					?>
					<div class="col-sm-3">
						<p><label>Select Declared By</label></p>
						<select class="form-control" name="declared_by" id="declared_by" style=" width:100% auto; ">
							<option value="">Choose Declarer</option>
							<?php 
								$get_product = $this->db->group_by('created_by')->get_where('product_ingredients');
								foreach($get_product->result() as $p){
			...
			<div class="box-header" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); ">
				<div class="row" style="padding:10px;">
					<div class="col-sm-12 text-right">
						...
						&nbsp;
						<button type="button" class="btn btn-primary btn-sm btn-flat" onclick="CSV.begin('#example').download('declared_ingredients.csv').go()" style=" box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2); "><i class="fa fa-file-excel-o"></i> Download CSV</button>
					</div>
				</div>
			</div><!-- /.box-header -->
			<div class="box-body">
...
<script type="text/javascript">
	$(function() {
		$("#example").dataTable({
			"processing": true,
			"serverSide": true,
			"ajax": "<?php echo base_url('Product_preparation/ingredientsListJson'); ?>"
		});
	});
...
<script type="text/javascript">
	function search_result()
	{
		...
		var mydata = {"product": product,"declared_by": declared_by,"sf_time": sf_time,"st_time": st_time};
		
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('Product_preparation/declared_search_ListJson'); ?>",
			data: mydata,
...
<script>
	...
	function createPDF(){
		getCanvas().then(function(canvas){
			...
			doc.save('declared_ingrediens.pdf');
