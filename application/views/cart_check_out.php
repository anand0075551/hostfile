...
<?php
	...
	$user_debit = $this->db->select_sum('debit')->get_where('accounts', ['points_mode'=>$points_mode, 'account_no' =>$account_no]); 
	
	foreach( $user_debit->result() 		as $user_debit);
	$users_debit			= $user_debit->debit;
	
	$user_credit = $this->db->select_sum('credit')->get_where('accounts', ['points_mode'=>$points_mode, 'account_no' =>$account_no]); 
...
<?php include('smb_header.php'); ?>
	<br><!-- /.box-header -->
		<div class="row alert alert-info box-header" style="margin:auto; box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2);">
		 ...
		 <span class="col-sm-6 text-left"><font size="4">ORDERS </font></span>
		 <span class="col-sm-6 text-right"><font size="3" color="indigo">Available CPA Balance : <?php echo number_format($user_balance,2) ?></font></span>
		</div>
	<div class="container">
			...
			?>
						...
						<tr>	
						 ...
						 <td width="5%" class="text-center" style="border:1px solid lightgray;">
						   ...
						   <td style="border:1px solid lightgray;"><?php 
								if($items['tax_percent']!=""){
						   ...
						   ?>%</td>
						   <td style="border:1px solid lightgray;"><?php 
								if($items['shipping_cost']!=""){
									echo $this->cart->format_number($items['shipping_cost']); 
								} 
								else{
									echo "0.00";
								}
					...
					<div class="row">
						<div class="col-md-12">
							<select class="form-control" name="voucher_id" id="voucher_id" style="width:100%" onchange="get_voucher_amount(this.value)" >
								...
								$data = $this->db->limit($limit,0)->where($condition)->get('vouchers');
				...
				<div class="row">
					<div class="col-md-12">
						<select class="form-control" name="voucher_id" id="voucher_id" style="width:100%" onchange="get_voucher_amount(this.value)" >
							...
							$today_date = date("Y-m-d"); 
							$condition =" voucher_name = '".$voc_type."' AND used = 'no' AND reserved = '' AND user_id = '".$user_id."' AND  start_date <= '".$today_date."' AND  end_date >= '".$today_date."' AND amount != 0 ";
							$data = $this->db->where($condition)->get('vouchers');
...
?>
		...
		<a class="btn btn-primary" id="next_address" onclick="get_address()"> Next </a>
		<!--< ?php } ?> -->
				...
				<br>
					...
					<div class="container row">
						<div class="col-md-12">
							  ...
							  <div class="tab-pane active" id="p_address" style="display:none;">
									<div class="row">
										<div class="col-md-1"></div>
							  ...
							  <div class="tab-pane" id="d_address">
									<div class="row">
										...
										<div class="col-md-3"><h6>Last Name <span class="text-red">*</span></h6>
										...
										<div class="col-md-3"><h6>Phone <span class="text-red">*</span></h6>
									...
									<div class="row">
										...
										<div class="col-md-3"><h6>Area Name <span class="text-red">*</span></h6>
									...
									<div class="row">
										<div class="col-md-1"></div>
										<div class="col-md-9"><h6>Address <span class="text-red">*</span></h6>
				 ...
				 <i class="fa fa-arrow-down" aria-hidden="true"></i>  <h4>PAYMENTS OPTIONS</h4>
					...
					<div class="row">
						...
						<div class="cc-selector col-sm-4">
							...
							<?php if($currency == "CPA"){ ?>
								<img src="<?php echo base_url(); ?>assets/payment_type/cpa.png" width="70%" height="70%" style=" text-align-last:center;" alt="..." class="img-thumbnail img-check" style="box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2);">
							<?php } elseif($currency == "LPA"){ ?>
								<img src="<?php echo base_url(); ?>assets/payment_type/lpa.png" width="70%" height="70%" style=" text-align-last:center;" alt="..." class="img-thumbnail img-check" style="box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2);">
							<?php }  else{ ?>
								<img src="<?php echo base_url(); ?>assets/payment_type/cpa.png" width="70%" height="70%" style=" text-align-last:center;" alt="..." class="img-thumbnail img-check" style="box-shadow: 0 2px 4px 0 rgba(0, 0, 0, .23), inset 1px 1px 0 0 hsla(0, 0%, 100%, .2);">
							<?php } ?>	
						...
						<div class="col-md-12 text-right" id="complete_order_div" style="padding-right:30px;">
						 ...
						 <?php if($sub_total != 0 ){ ?>
							<div class="row text-center" id="complete_order_btn_div" style="display:none;">
								...
								<div class="col-md-4">
									<button id="complete_order_btn" type="submit"  onclick="clicked();"  name="submit" value="check_item"  class="btn btn-success disabled" > Comfirm Your Order By Deducting Balance </button>
								</div>
							</div>
						 <?php } ?>
						 
						<label id="increase_balance_btn" style="display:none;">
							<?php 
							if($currency == 'CPA'){
							?>
								<a href="<?php echo base_url('bank/add_bankdeposit')?>" class="btn btn-danger">Please Increase Your CPA Balance</a> 
							<?php
							} 
							if($currency == 'LPA'){
								echo '<font color="red" size="4"> Use Cfirst Services Regularly To Avail Your  Loyality Balance..!</font>';
							} 
						 ?>
...
<script>
	function check_balace()
	{
		...
		var payable_amount = $("#grand_total").val();
		...
		if(sts>0){
			...
			$("#complete_order_btn").html("Comfirm Your Order By Deducting "+payable_amount+" Values");
		}
		else{
			$("#balance_check_btn").hide();
