  ...
  /**
    ...
    public function earningListCount(){
        ...
        if($user['role'] == 'admin')
        {
		   ...
		   $query = $this->db->where('user_id',$userID)->count_all_results('accounts');
        }
...
//***************************************************************************************************	
//Anand Account statements
       public function totalAssets(){
        $user = loggedInUserData();
        $userID = $user['user_id'];

   
           $query = $this->db->select_sum('amount')->where('pay_type', 'transfer')->get('ledger');
...
//***************************************************************************************************
    ...
    public function make_payment(){
        $customer_referral_id = $this->input->post('payeeID');
        $customer_info = singleDbTableRow($customer_referral_id, 'users');
