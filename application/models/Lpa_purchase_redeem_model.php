...
class Lpa_purchase_redeem_model extends CI_Model {
	...
	public function invoiceListCount(){
        ...
        $userID = $user['user_id'];
			...
			$query = $this->db->get_where('invoice', ['sales_by'=>$userID, 'customer_id'=>1259])->num_rows();       
    ...
    public function invoiceList($limit = 0, $start = 0 ){
		...
		if ($searchValue != '') {
			$table_name = "invoice";
			$where_array = " sales_by = '".$userID."' AND id LIKE '%".$searchValue."%' AND customer_id = 1259 ";
			$query = $this->db->order_by('id', 'desc')->limit($limit, $start)->where($where_array)->get($table_name);
		}
		else{
			$query = $this->db->order_by('id', 'desc')->limit($limit, $start)->get_where('invoice', ['sales_by'=>$userID, 'customer_id'=>1259]);
		}
	...
	public function reedem_invoiceListCount(){
        $user = loggedInUserData();
        $userID = $user['user_id'];
			
			$query = $this->db->get_where('invoice', ['sales_by'=>1259, 'created_by'=>$userID])->num_rows();       
    ...
    public function reedem_invoiceList($limit = 0, $start = 0 ){
		$user = loggedInUserData();
		$userID = $user['user_id'];
		...
		if ($searchValue != '') {
			$table_name = "invoice";
			$where_array = " created_by = '".$userID."' AND id LIKE '%".$searchValue."%' AND sales_by = 1259 ";
			$query = $this->db->order_by('id', 'desc')->limit($limit, $start)->where($where_array)->get($table_name);
		}
		else{
			$query = $this->db->order_by('id', 'desc')->limit($limit, $start)->get_where('invoice', ['sales_by'=>1259, 'created_by'=>$userID]);
		}
...
//For Transaction Series	
	...
	public function retailer_payment(){
		...
		//$paying_to  		= $this->input->post('referredByCode');		//Paying To Consumer ID
		$amount	    		= $this->input->post('cpa_amount');		//Paying Amount	
		//$paid_by			= $user_referral_code;		//Paying By Consumer ID			
		$pay_type   		= '93';   //Pay Specification
		$tranx_id   		= $this->input->post('tranx_id');
		//$transaction_type 	= $this->input->post('conv_type');
...
//Very Important changing parties for send/receive based on Radio button selection		
			//for recive payment.
			$pay_by = '2157983604';
			$pay_to = $this->input->post('referredByCode');
...
$acct_user 		= $c_id;
$result_count  	= $this->lpa_purchase_redeem_model->get_tran_count($acct_user);		
...
/*	Get user Balance and Update Final Amount */
		...
		$user_balance  	= $this->lpa_purchase_redeem_model->user_balance($bal_userID, $points_mode);
	...
	//Sender/Seller/Active user
								...
								$user_debit  	= $this->lpa_purchase_redeem_model->total_debit($userID, $points_mode);
								$user_credit  	= $this->lpa_purchase_redeem_model->total_credit($userID, $points_mode);
...
//**********************************************************************************************************//
	...
	public function saleListCount() {
		...
		$query = $lpa_db->where('user_id', $user_id)->count_all_results('orders');
		return $query; 
		
    }	
	
	public function salelist($limit = 0, $start = 0) {
		...
		if ($searchValue != '') {
			$table_name = "orders";
			$where_array = " user_id = '".$user_id."' AND transaction_id LIKE '%".$searchValue."%' ";
			$query = $lpa_db->order_by('id', 'desc')->limit($limit, $start)->where($where_array)->get($table_name);
		}
		else{
			$query = $lpa_db->order_by('id', 'desc')->limit($limit, $start)->get_where('orders', ['user_id'=>$user_id]); 
		}
