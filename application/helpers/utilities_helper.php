...
/**
 * @param int Bank ID List
 * @return mixed
 */

if( ! function_exists('get_profile_by_id2'))
{
    function get_profile_by_id2($user_id = 1){
        $CI =& get_instance();
        $query = $CI->db->get_where('bank', ['id' => $user_id]);
        if($query->num_rows() > 0)
        {
            foreach($query->result() as $profile);
            return $profile;
...
if( ! function_exists('ledger_photo_url'))
{
    function ledger_photo_url($reciept_name = '', $email = 'example@example.com'){
        $url = '';
        if($reciept_name != '')
        {
            $url .= base_url('uploads/'.$reciept_name);
        }
        elseif($email != '')
        {
            $url .= "http://www.gravatar.com/avatar/" . md5( strtolower( trim( $email ) ) );
...
if( ! function_exists('singleDbTableRow1'))
{
    function singleDbTableRow1($id = 1, $table = 'users', $column = 'cid'){
        $CI =& get_instance();
        $query = $CI->db->get_where($table, [$column => $id ]);
        if($query->num_rows() > 0)
        {
...
if( ! function_exists('bgform_id'))
{
    function bgform_id($id = 1, $table = 'users', $column = 'bgform_id'){
        $CI =& get_instance();
        $query = $CI->db->get_where($table, [$column => $id ]);
        if($query->num_rows() > 0)
        {
 ...
 * @param string $column
 * @return bool
 * @return Account/Ledger row by give table id or false
  Example Code:"	$roleName = ledgerDbTableRow($profile->rolename)->rolename; "
 */

if( ! function_exists('ledgerDbTableRow'))
{
    function ledgerDbTableRow($id = 1, $table = 'acct_categories', $column = 'id'){
        $CI =& get_instance();
        $query = $CI->db->get_where($table, [$column => $id ]);
        if($query->num_rows() > 0)
        {
 ...
 /* @return Commissions/Vouchers row by give table id or false
 */

if( ! function_exists('commissionDbTableRow'))
{
    function commissionDbTableRow($id = 1, $table = 'commissions', $column = 'id'){
        $CI =& get_instance();
        $query = $CI->db->get_where($table, [$column => $id ]);
        if($query->num_rows() > 0)
        {
 ...
 * @Anand int $id
 * @param string $table
 * @return product/invoice row by give table id or false
 Example Code:"	$roleName = typeDbTableRow($profile->rolename)->rolename; "
 */

if( ! function_exists('invoiceDbTableRow'))
{
    function invoiceDbTableRow($id = 1, $table = 'invoice', $column = 'id'){
        $CI =& get_instance();
        $query = $CI->db->get_where($table, [$column => $id ]);
        if($query->num_rows() > 0)
        {
...
/**
 * @Anand int $id
 * @return Description for User/Loan/Account Types
 */

if( ! function_exists('typeDbTableRow'))
{
    function typeDbTableRow($id = 1, $table = 'role', $column = 'id'){
        $CI =& get_instance();
        $query = $CI->db->get_where($table, [$column => $id ]);
        if($query->num_rows() > 0)
        {
...
/**
 * @Anand int $id
 * @return Description for Bill Pay Error description
 */

if( ! function_exists('billpayDbTableRow'))
{
    function billpayDbTableRow($id = 1, $table = 'error_table', $column = 'code'){
        $CI =& get_instance();
        $query = $CI->db->get_where($table, [$column => $id ]);
        if($query->num_rows() > 0)
        {
...
if( ! function_exists('permittedArea'))
{
    function permittedArea($permission = ['admin']){
        ...
        if( ! in_array($authDta['role'], $permission))
        {
            ...
            //show_error('Consumer! You are not permitted in this area');
            $CI->session->set_flashdata('errorMsg', 'Dear Consumer! You are not permitted in this area');
