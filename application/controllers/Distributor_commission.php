	...
	public function my_commission()
	{
		...
		if($commission -> num_rows() > 0) 
		{
			...
			foreach ($commission->result() as $commissions) 
			{
				...
				/* area */
				$get_location = $this->db->get_where('area', ['id'=>$area]);
				foreach($get_location->result() as $l);
				$location=$l->location;
				$get_location_name = $this->db->get_where('location_id', ['id'=>$location]);
				foreach($get_location_name->result() as $ln);
			...
			foreach ($period as $dt) {
				...
				{
					/*check paid */
					$check_paid = $this->Distributor_commission_model->Check_paid($from_role,$first_day,$last_day,$business_group);
					/*Total Sale */
				...
				else
				{
					...
					$check = $this->Distributor_commission_model->Check_upper_commission($from_role,$first_day,$last_day,$business_group);
					if($check -> num_rows() > 0) 
					{
				...
				$data['data'][] = array(
						...
						$ln->location,
						singleDbTableRow($from_role, 'role')->rolename,
						$total_users,
