...
$this->load->model('notification_model');
	...
	public function addAdmin(){
		...
		if($this->input->post())
		{
			...
			$this->form_validation->set_rules('rolename', 'User Role Name', 'required');			
			...
			if($this->form_validation->run() == true)
			{
				...
				if($insert)
				{
					$this->session->set_flashdata('successMsg', 'New Administrator Created Successfully!');
...
/**
	...
	public function add_roles(){
		...
		if($this->input->post())
		{
			...
			if($this->form_validation->run() == true)
			{
				$insert = $this->settings_model->add_roles();
				if($insert)
				{
					$this->session->set_flashdata('successMsg', 'New User-Role Created Successfully!');
					redirect(base_url('admin_settings/add_roles'));
				}
...
/**
	...
	public function all_roles(){
		...
		//$data['adminList'] = $this->db->order_by('id', 'DESC')->get_where('users', ['role' => 'admin']);
		$data['role_List'] = $this->db->order_by('id', 'DESC')->get_where('role', ['type' => 'role_name']);
...
/**
	 * @all_RolesPermission Assigned to All levels of User
	 */

	public function authorizations(){
		//restricted this area, only for admin
		//permittedArea();
		//$data['permissions'] = $this->Settings_model->permissions($limit, $start);
		
		theme('authorizations');
	}
}
